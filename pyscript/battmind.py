    ...
    "template": [
        {
            "sensor": [
                ...
                {
                    "default_entity_id": f"sensor.{__name__}_battery_level_needed",
                    "unique_id": f"{__name__}_battery_level_needed",
                    "icon": "mdi:chart-bell-curve",
                    "unit_of_measurement": "%",
                    "state": "unavailable"
                }
...
def get_min_sell_kwh():
    ...
    _LOGGER = globals()['_LOGGER'].getChild(func_name)
    try:
        return float(get_state(f"input_number.{__name__}_min_sell_kwh", float_type=True, error_state=None))
    except Exception as e:
...
def cheap_grid_charge_hours(force_recalculate = False):
    ...
    def battery_level_flow_prediction_recalc(final_recalc=False):
        ...
        if final_recalc:
            ...
            battery_level_needed_sum = max(min(battery_level_needed_sum, CONFIG['solar']['powerwall_battery_level_max']), CONFIG['solar']['powerwall_battery_level_min'])
            ...
            set_attr(f"sensor.{__name__}_battery_level_needed.battery_level_flow_prediction", battery_level_dict)
    ...
    def future_charging(totalCost, totalkWh):
        ...
        def cheapest_hour_fill_planner(day):
            ...
            try:
                ...
                highest_battery_level_timestamp = charging_plan[day]["start_of_day"]
                ...
                for hour in range(0, 24):
                    ...
                    if battery_level > highest_battery_level:
                        ...
                        highest_battery_level_timestamp = charging_plan[day]["start_of_day"].replace(hour=hour)
        ...
        def most_expensive_planner(day):
            ...
            try:
                for sorted_hour in charging_plan[day]['sorted_hour_cost_prediction'][FORECAST_TYPE]:
                    ...
                    remove_list = []
                    highest_battery_level = sorted_battery_level
                    ...
                    for timestamp, price in sorted_by_cheapest_price:
                        ...
                        what_day = daysBetween(current_hour, timestamp)
                        highest_battery_level = max(sum(charging_plan[what_day]['battery_level_flow'].get(timestamp.hour, [0.0])), highest_battery_level)
                        
                        for i in range(1, hoursBetween(timestamp, sorted_timestamp) + 1):
                            loop_timestamp = (sorted_timestamp - datetime.timedelta(hours=i))
                            loop_day = daysBetween(current_hour, loop_timestamp)
                            loop_battery_level = sum(charging_plan.get(loop_day, {}).get('battery_level_flow', {}).get(loop_timestamp.hour, [0.0]))
                            
                            
                            if loop_battery_level > highest_battery_level:
                                highest_battery_level_timestamp = loop_timestamp
                                highest_battery_level = max(loop_battery_level, highest_battery_level)
                        ...
                        if round(kwh_needed, 1) > 0.0 and kwh_to_percentage(kwh_needed, include_charging_loss = True) > 0.0:
                            ...
                            if timestamp in chargeHours and battery_level_added:
                                ...
                                reason = (
                                    ...
                                    f"Prioriteret elpris: **{sorted_price}** {i18n.t('ui.common.valuta_kwh')}<br>"
                                    f"Højeste batteriniveau tidspunkt: **{highest_battery_level_timestamp.strftime('%H:%M')}**<br>"
                                    f"Højeste batteriniveau: **{highest_battery_level:.1f}%**<br>"
                                    f"Aktuel elpris: **{price} {i18n.t('ui.common.valuta_kwh')}**<br>"
                                    f"Profit ved at lade til denne time: **{kwh_profit:.2f} {i18n.t('ui.common.valuta_kwh')}**<br>"
            ...
            for hour in range(24):
                ...
                if profit < min_profit_per_kwh:
                    ...
                    reason = (
                        ...
                        f"Aktuel elpris: **{price} {i18n.t('ui.common.valuta_kwh')}**<br>"
                        f"Minimum fortjeneste pr. kWh: **{min_profit_per_kwh:.2f} {i18n.t('ui.common.valuta_kwh')}**<br>"
                        f"Fortjeneste pr. kWh: **{profit:.2f} {i18n.t('ui.common.valuta_kwh')}**<br>"
                        "</details>"
                        )
        ...
        def most_profit(day):
            ...
            if not use_midnight_battery_level_enabled():
                if day < amount_of_days - 1:
                    for hour in range(24):
                        timestamp = charging_plan[day + 1]['start_of_day'] + datetime.timedelta(hours=hour)
                        ...
                        battery_level = sum(charging_plan[what_day]['battery_level_flow'].get(loop_hour, None))
            ...
            for i in range(hours + 1):
                ...
                powerwall_usage_profit = grid_price - battery_kwh_cost if grid_price > 0.0 else 0.0
                powerwall_usage_profit *= charging_plan[what_day]['hour_cost_prediction'][FORECAST_TYPE][hour]['kwh']
                
                if powerwall_usage_profit < min_profit_per_kwh:
                    exclude_hours.append(hour)
                    continue
            ...
            for i in range(hours + 1):
                ...
                grid_sell_profit = grid_sell_price - battery_kwh_cost if grid_sell_price > 0.0 else 0.0
                grid_sell_profit *= kwh
                
                grid_sell_price = round(grid_sell_price, 2)
                ...
                if sell_profit > min_sell_profit:
                    ...
                    charging_plan[what_day]["force_discharge_timestamps"][timestamp] = {
                        ...
                        "reason": (
                            ...
                            f"Wear cost per kWh: **{abs(CONFIG['solar']['powerwall_wear_cost_per_kwh']):.2f} {i18n.t('ui.common.valuta_kwh')}**<br>"
                            f"**Samlet battery kWh cost**: **{battery_kwh_cost:.2f} {i18n.t('ui.common.valuta_kwh')}**<br>"
                            f"Grid sell price: **{fixed_price_calc_text}{grid_sell_price:.2f} {i18n.t('ui.common.valuta_kwh')}**<br>"
                            f"kWh sold: **{kwh:.2f} kWh**<br>"
                            f"Profit from selling: **{grid_sell_profit:.2f} {i18n.t('ui.common.valuta')}**<br>"
                            f"Profit from using battery: **{sum(powerwall_usage_profit_total):.2f} {i18n.t('ui.common.valuta')}**<br>"
                            f"Difference: **{diff:.2f} {i18n.t('ui.common.valuta')}**<br>"
                            "</details>"
                        ),
                        }
        ...
        sell_rules = {
            1: {
                "name": "most_profit",
                "enabled_func": most_profit_enabled(),
                "func": most_profit
            },
            2: {
    ...
    for day in range(amount_of_days):
        ...
        for hour in range(from_hour, 24):
            ...
            charging_plan[day]['hour_cost_prediction']['average'][hour] = {
        ...
        forecast_types = ['ema', 'trend', 'average']
    ...
    for day in charging_plan.keys():
        ...
        for hour in hour_cost_prediction['average'].keys():
            timestamp = start_of_day + datetime.timedelta(hours=hour)
            
            avg = hour_cost_prediction['average'][hour]['kwh']
    ...
    try:
        ...
        for day in charging_plan.keys():
            ...
            avg = ", ".join([f"{hour:02d}" for hour in charging_plan[day]['grouped_sorted_hour_cost_prediction']['average']])
    ...
    try:
        ...
        for day in charging_plan.keys():
            ...
            for hour in charging_plan[day]['grouped_sorted_hour_cost_prediction']["average"]:
                hours = charging_plan[day]['grouped_hour_cost_prediction']["average"][hour]['hours']
                kwh = charging_plan[day]['grouped_hour_cost_prediction']["average"][hour]['total_kwh']
                cost = charging_plan[day]['grouped_hour_cost_prediction']["average"][hour]['total_cost']
                
                price = charging_plan[day]['grouped_hour_cost_prediction']["average"][hour]['avg_price']
    ...
    try:
        ...
        for day in charging_plan.keys():
            ...
            avg = ", ".join([f"{hour:02d}" for hour in charging_plan[day]['sorted_hour_cost_prediction']['average']])
    ...
    try:
        ...
        for day in charging_plan.keys():
            ...
            for hour in charging_plan[day]['hour_cost_prediction']["ema"].keys():
                ...
                avg_kwh = charging_plan[day]['hour_cost_prediction']["average"][hour]['kwh']
                avg_cost = charging_plan[day]['hour_cost_prediction']["average"][hour]['cost']
...
def local_energy_prediction(powerwall_charging_timestamps = False):
    ...
    for key in output.keys():
        if key not in ("average", "today") and output[key] is not None:
                avg.append(output[key])
    output['average'] = average(avg)

    for key in output_sell.keys():
        if key not in ("average", "today") and output_sell[key] is not None and output[key] != 0.0:
                avg_sell.append(output_sell[key])
    output_sell['average'] = average(avg_sell)
    
    for date in output.keys():
        if output[date] is None:
            output[date] = output['average']
            
    for date in output_sell.keys():
        if output_sell[date] is None:
            output_sell[date] = output_sell['average']
